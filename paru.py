...
class Paru(dotbot.Plugin):
    ...
    def handle(self, directive, data):
        if not self.can_handle(directive):
            raise ValueError('Paru cannot handle directive %s' % directive)
        if self._bootstrap_paru() != 0:
            raise Exception('Paru could not be installed on your system')
    ...
    def _bootstrap_paru(self):
        dir_path = os.path.dirname(os.path.realpath(__file__))
        cmd = '{}/bootstrap-paru'.format(dir_path)
